...
class CAMR(object):
    ...
    def rename_node(self, prefix):
        """
        Rename CAMR graph nodes to prefix + node_index to avoid nodes with the
        same name in two different CAMRs.
        """
        node_map_dict = {}
    ...
    def get_triples(self):
        ...
        instance_triple: a triple representing an instance.
        E.g. instance(a0, 爱-01), instance(a1, name)
        attribute triple: links of attributes,
        e.g. anchor(a0, x2), op1(a1, 北京)
        ...
        # print('*'*50 + f'\ninstance: {instance_triple} \n\n attr: {attribute_triple} \n\n rel: {relation_triple} \n')
        # input()
    ...
    def get_CAMR(input_f, len_dict=None, smatch=True):
        ...
        for line in input_f:
            line = line.strip()
            if line != '':
                [sid, nid1, concept1, coref1, rel, rid, ralign,
                    nid2, concept2, coref2] = line.split('\t')
                ...
                result_camr = CAMR(node_list, node_value_list,
                                   relation_list, anchor_list, sid)
                return result_camr
