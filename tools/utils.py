...
def get_best_amr_match(cur_amr1, cur_amr2):
    def format_inss(inss):
        # amr转化中，会将"ins"处理为ins_，需要逆转回来
        n_inss = []
        for x, rel, y in inss:
            if y.endswith("_"):
                n_y = f"\"{y[:-1]}\""
                n_inss.append((x, rel, n_y))
            else:
                n_inss.append((x, rel, y))
        return n_inss
    amr_pair = []
    ...
    instance1 = format_inss(instance1)
    instance2 = format_inss(instance2)
