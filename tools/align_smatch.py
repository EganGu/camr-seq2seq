    ...
    for i in range(iteration_num):
        ...
        if i == 0:
            ...
            cur_mapping = smart_init_mapping(
                candidate_mappings, instance1, instance2)
        else:
        ...
        while True:
            ...
            if veryVerbose:
                print("Update triple match number to:",
                      match_num, file=DEBUG_LOG)
...
def compute_match(mapping, weight_dict):
    ...
    if tuple(mapping) in match_triple_dict:
        if veryVerbose:
            print("saved value", match_triple_dict[tuple(
                mapping)], file=DEBUG_LOG)
    ...
    for i, m in enumerate(mapping):
        ...
        for key in weight_dict[current_node_pair]:
            if key == -1:
                ...
                if veryVerbose:
                    print("instance/attribute match",
                          weight_dict[current_node_pair][key], file=DEBUG_LOG)
    ...
    for i, nid in enumerate(mapping):
        ...
        for nm in unmatched:
            if nm in candidate_mappings[i]:
                ...
                # (i, m) -> (i, nm)
                if veryVerbose:
                    print("Remap node", i, "from ",
                          nid, "to", nm, file=DEBUG_LOG)
                mv_gain = move_gain(mapping, i, nid, nm,
                                    weight_dict, cur_match_num)
                if veryVerbose:
    ...
    for i, m in enumerate(mapping):
        for j in range(i + 1, len(mapping)):
            ...
            if veryVerbose:
                ...
                print("Before swapping:", i, "-", m,
                      ",", j, "-", m2, file=DEBUG_LOG)
                print(mapping, file=DEBUG_LOG)
                print("After swapping:", i, "-", m2,
                      ",", j, "-", m, file=DEBUG_LOG)
            sw_gain = swap_gain(mapping, i, m, j, m2,
                                weight_dict, cur_match_num)
            if veryVerbose:
...
def generate_amr(f1, f2, sid_len):
    ...
    """
    while True:
        cur_amr1 = CAMR.get_CAMR(f1, sid_len, smatch=False)
        cur_amr2 = CAMR.get_CAMR(f2, sid_len, smatch=False)
        if not cur_amr1 and not cur_amr2:
            ...
            print("Error: Sentence "+cur_amr1.sid +
                  " has different sid with "+cur_amr2.sid, file=ERROR_LOG)
        else:
...
def get_amr_match(camr1, camr2, sent_num, justinstance=False, justattribute=False, justrelation=False):
    ...
    if verbose:
        ...
        print("Best node mapping alignment:", print_alignment(
            node_mapping, instance1, instance2), file=DEBUG_LOG)
    if justinstance:
...
def score_amr_pairs(f1, f2, leng, justinstance=False, justattribute=False, justrelation=False):
    ...
    for cur_amr1, cur_amr2 in tqdm(generate_amr(f1, f2, sid_len)):
...
def main(arguments):
    ...
    for (precision, recall, best_f_score), sid in score_amr_pairs(arguments.f[0], arguments.f[1], arguments.lf,
        ...
        if pr_flag:
            if not single_score:
                print(sid, "Precision: " + floatdisplay % precision, " Recall: " +
                      floatdisplay % recall, " F-score: " + floatdisplay % best_f_score)
            else:
                print("Precision: " + floatdisplay % precision, " Recall: " +
                      floatdisplay % recall, " F-score: " + floatdisplay % best_f_score)
