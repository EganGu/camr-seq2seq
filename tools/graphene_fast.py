...
import argparse
import penman
# from amrlib.evaluate.smatch_enhanced import match_pair as match_pair_amr
from amrlib.evaluate.smatch_enhanced import compute_smatch
import smatch
from utils import match_pair, align, get_entries
import re
...
#     for (n1, n2, n3) in [match_pair_amr(p) for p in pairs]:
#         mum_match += n1
#         mum_test  += n2
#         mum_gold  += n3
...
def ensemble(amrs, threshold):
    ...
    ensemble_support = get_total_support(
        instances, attributes, relations, g, freq)
    original_support = get_total_support(
        instances, attributes, relations, original_g, freq)
...
def vote(test_entries, gold_entries, threshold):
    ...
    for j in range(len(test_entries[0])):
        amrs = [test_entries[line][j] for line in range(len(test_entries))]
...
def graphene(test_entries, gold_entries, threshold, ties_broken_arbitrarily='no'):
    def g_pivot_iter(voters, gold_entries, threshold, i, n):
        print("Pivot iteration starts:", i, "/", n)
        ensemble_entries, scores, _, _, _, _ = vote(
            voters, gold_entries, threshold)
        print(sum(scores) / len(scores))
        return ensemble_entries, scores
    ...
    def generate_voters():
        for i in range(len(test_entries)):
            ...
            yield voters, gold_entries, threshold, i, len(test_entries)-1

    with mp.Pool(MAX_CPU) as pool:
        results = [pool.apply_async(g_pivot_iter, args)
                   for args in generate_voters()]
        all_results = list(r.get() for r in results)

    all_ensembles = [x[0] for x in all_results]
    all_scores = [x[1] for x in all_results]

    def g_get_best_n(i, all_ensembles):
        bs = None
        idx = None
        best_indices = []
        ...
        idx = best_indices[0]
        return all_ensembles[idx][i], bs, idx

    with mp.Pool(MAX_CPU) as pool:
        results = [pool.apply_async(g_get_best_n, (n, all_ensembles))
                   for n in range(len(all_ensembles[0]))]
        all_results = list(r.get() for r in results)

    best_entries = [x[0] for x in all_results]
    best_score = [x[1] for x in all_results]
    vote_idx = [x[2] for x in all_results]
...
def graphene_smatch(test_entries, gold_entries, threshold):
    ...
    def pivot_iter(voters, gold_entries, threshold, i, n):
        print("Pivot iteration starts:", i, "/", n)
        ensemble_entries, scores, _, _, _, _ = vote(
            voters, gold_entries, threshold)
        print(sum(scores) / len(scores))
    ...
    with mp.Pool(MAX_CPU) as pool:
        results = [pool.apply_async(pivot_iter, args)
                   for args in generate_voters()]
        all_ensembles = list(r.get() for r in results)

    for i in range(len(test_entries)):
        all_ensembles.append(test_entries[i])

    def get_best_n(n, all_ensembles, test_entries):
        ...
        for i in range(len(all_ensembles)):
            ...
            _, _, f_score = compute_smatch(graphs, others)
            if best_f_score is None or best_f_score < f_score:
                best_f_score = f_score
                best_graph = all_ensembles[i][n]
        if n % 10 == 0:
            print(f"{n} / {len(test_entries[0])}")
        return best_graph

    with mp.Pool(MAX_CPU) as pool:
        results = [pool.apply_async(get_best_n, (n, all_ensembles, test_entries))
                   for n in range(len(all_ensembles[0]))]
        best_entries = list(r.get() for r in results)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Graph Ensemble (Graphene) for AMR parsing')
    ...
    print(data)
    gen_fname = [f"{data}/{p}" for p in os.listdir(data)]
    ...
    for gen_file in gen_fname:
        ...
        if args.align == 'no':
            ...
            test = align(original_gold_entries,
                         original_test_entries_1, test_entries_1)
        else:
            test = test_entries_1
    ...
    with mp.Pool(MAX_CPU) as pool:
        results = [pool.apply_async(
            compute_smatch, (test, gold_entries)) for test in test_entries]
        for res in results:
            print(' SMATCH -> P: %.3f,  R: %.3f,  F: %.3f' % res.get())
    ...
    best_prediction = [penman.encode(penman.decode(g))
                       for g in ensemble_entries]
